...
use App\Models\Workflows\WorkflowMetric;
use App\Models\Workflows\WorkflowRun;
use App\Models\Workflows\WorkflowSecret;
...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Configure SSRF protection and response limits for the HTTP Request action.
    |
    */
    'http_request' => [
        'blocked_domains' => [],
        'allowed_domains' => [], // empty = allow all non-private
    ...
    */
    'tenancy' => [
        ...
        'column' => env('WORKFLOWS_TENANCY_COLUMN', 'user_id'),

        // The tenant model (optional, for reference)
        'model' => env('WORKFLOWS_TENANT_MODEL', 'App\\Models\\User'),
    ],
    ...
    */
    'integrations' => [
        ...
        'model_states' => env('WORKFLOWS_MODEL_STATES_ENABLED', true),
        'actions_bridge' => env('WORKFLOWS_ACTIONS_BRIDGE_ENABLED', true),
        'webhook_client' => env('WORKFLOWS_WEBHOOK_CLIENT_ENABLED', true),

        // Action classes available to the RunAction workflow step.
        // Each must have an execute() method.
        'action_classes' => [
            // App\Actions\ResolvePharmacyFromDispense::class,
        ],
    ],
